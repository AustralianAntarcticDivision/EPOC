# Create S4 method 'reproduceSetup'
if (!isGeneric("reproduceSetup")) 
setGeneric("reproduceSetup", function(element, period="Period", ptSA="list", modulenum="numeric", elementnum="numeric", 
										firstPeriod="numeric", dset="list") standardGeneric("reproduceSetup"))
setMethod("reproduceSetup", "Predator", 
    function(
	...
	{
		# Function:           B.Pr.KPFM.Reproduce.setup.01
		# Version             0.01
		...
		for (st in 1:getState(element, "StageN")) ptSA[[1]]$dset$M <- ptSA[[1]]$dset$M * adj
